...
// Emits observability events to dripops-splunk-hec-bridge via service binding.
// ─────────────────────────────────────────────────────────────────────
function emitDripops(env, eventType, severity, fields) {
  if (!env.DRIPOPS_INGEST_KEY) return Promise.resolve();
  const payload = JSON.stringify({
    source: 'tuck-cache-refresh',
  ...
  const p = useBinding
    ...
    : fetch('https://dripops-splunk-hec-bridge.thom-rvr.workers.dev/event',
        { method: 'POST', headers, body: payload, signal: AbortSignal.timeout(5000) });
  return p
    .then((r) => { console.log('[dripops] emit ' + eventType + ' → ' + r.status); })
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (path === '/refresh' && request.method === 'POST') {
      ...
      // DRIPOPS_INSTRUMENTATION_V1 — emit on manual refresh fires too
      const dripopsOk = result.macro_keys >= 5 && result.prices_tickers >= 8;
      ctx.waitUntil(emitDripops(env, 'cache_refreshed', dripopsOk ? 'info' : 'warning', {
        elapsed_ms: result.elapsed_ms,
        macro_keys: result.macro_keys,
        prices_tickers: result.prices_tickers,
        tucks_score_count: result.tucks_score_count,
        sector_count: result.sector_count,
        trigger: 'manual',
        passed_health_check: dripopsOk,
      }));
      return j(result);
    }
    ...
    if (path === '/preview' && request.method === 'GET') {
      ...
      // DRIPOPS_INSTRUMENTATION_V1 — freshness drift detection (cache older than 8h = problem)
      const now = Date.now();
      const ageMs = (cached) => {
        if (!cached || !cached._cached_at) return null;
        const ts = Date.parse(cached._cached_at);
        return isNaN(ts) ? null : now - ts;
      };
      const macroAge = ageMs(macro);
      const pricesAge = ageMs(prices);
      const MAX_AGE_MS = 8 * 60 * 60 * 1000;
      const drifted = (macroAge !== null && macroAge > MAX_AGE_MS) ||
                      (pricesAge !== null && pricesAge > MAX_AGE_MS);
      if (drifted) {
        ctx.waitUntil(emitDripops(env, 'freshness_drift', 'warning', {
          macro_age_ms: macroAge,
          prices_age_ms: pricesAge,
  ...
  async scheduled(event, env, ctx) {
    ctx.waitUntil((async () => {
      ...
      let result;
      try {
        result = await refreshAll(env);
      } catch (err) {
        // DRIPOPS_INSTRUMENTATION_V1 — refresh blew up
        await emitDripops(env, 'refresh_failed', 'error', {
          error_message: String(err && err.message || err).slice(0, 500),
          cron_event: event && event.cron,
        });
        throw err;
      }
      console.log('[tuck-cache-refresh] done:', JSON.stringify(result).slice(0,500));
      
      // DRIPOPS_INSTRUMENTATION_V1 — emit success metrics
      const dripopsOk = result.macro_keys >= 5 && result.prices_tickers >= 8;
      ctx.waitUntil(emitDripops(env, 'cache_refreshed', dripopsOk ? 'info' : 'warning', {
        elapsed_ms: result.elapsed_ms,
        macro_keys: result.macro_keys,
