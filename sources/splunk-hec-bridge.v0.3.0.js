...
async function splunkSavedSearchViaRelay(env, savedSearchName, app = "search", owner = "nobody", earliest, latest) {
  if (!env.SSHMCP_RELAY_URL || !env.SSHMCP_SECRET) {
    return { ok: false, status: 0, body: "relay not configured" };
  }
  if (!env.SPLUNK_API_URL || !env.SPLUNK_SEARCH_TOKEN) {
    return { ok: false, status: 0, body: "splunk search not configured" };
  }
  // Dispatch the saved search → returns a search ID (sid)
  const dispatchPath = `/servicesNS/${owner}/${app}/saved/searches/${encodeURIComponent(savedSearchName)}/dispatch`;
  const dispatchParams = new URLSearchParams({ output_mode: "json" });
  if (earliest) dispatchParams.set("trigger_actions", "0");
  if (earliest) dispatchParams.set("dispatch.earliest_time", earliest);
  if (latest) dispatchParams.set("dispatch.latest_time", latest);
  const dispatchBody = dispatchParams.toString().replace(/'/g, "'\\''");
  const authEsc = env.SPLUNK_SEARCH_TOKEN.replace(/'/g, "'\\''");
  const dispatchCmd = `curl -sS -k --max-time 30 -X POST '${env.SPLUNK_API_URL}${dispatchPath}' -H 'Authorization: Bearer ${authEsc}' -H 'Content-Type: application/x-www-form-urlencoded' --data '${dispatchBody}'`;
  const dispR = await fetch(env.SSHMCP_RELAY_URL, {
    method: "POST",
    headers: { "X-SSH-Secret": env.SSHMCP_SECRET, "Content-Type": "application/json" },
    body: JSON.stringify({ cmd: "bash", args: ["-c", dispatchCmd] })
  });
  if (!dispR.ok) return { ok: false, status: dispR.status, body: `relay gateway error` };
  const dispJ = await dispR.json().catch(() => null);
  if (!dispJ || dispJ.exitCode !== 0) return { ok: false, status: 0, body: `dispatch failed: ${JSON.stringify(dispJ)}` };
  let sid;
  try {
    const parsed = JSON.parse(dispJ.stdout || "{}");
    sid = parsed.sid;
  } catch {
    return { ok: false, status: 0, body: `dispatch parse failed: ${(dispJ.stdout||"").slice(0,200)}` };
  }
  if (!sid) return { ok: false, status: 0, body: `no sid returned: ${(dispJ.stdout||"").slice(0,200)}` };

  // Poll for completion (up to 25s)
  let attempts = 0;
  while (attempts < 12) {
    await new Promise(r => setTimeout(r, attempts === 0 ? 1000 : 2000));
    const statusCmd = `curl -sS -k --max-time 10 '${env.SPLUNK_API_URL}/services/search/jobs/${sid}?output_mode=json' -H 'Authorization: Bearer ${authEsc}'`;
    const sR = await fetch(env.SSHMCP_RELAY_URL, {
      method: "POST",
      headers: { "X-SSH-Secret": env.SSHMCP_SECRET, "Content-Type": "application/json" },
      body: JSON.stringify({ cmd: "bash", args: ["-c", statusCmd] })
    });
    if (!sR.ok) { attempts++; continue; }
    const sJ = await sR.json().catch(() => null);
    if (!sJ || sJ.exitCode !== 0) { attempts++; continue; }
    try {
      const st = JSON.parse(sJ.stdout || "{}");
      const entry = st.entry && st.entry[0];
      const done = entry && entry.content && entry.content.dispatchState === "DONE";
      if (done) break;
    } catch {}
    attempts++;
  }
  ...
  // Fetch results
  const resultsCmd = `curl -sS -k --max-time 20 '${env.SPLUNK_API_URL}/services/search/jobs/${sid}/results?output_mode=json&count=500' -H 'Authorization: Bearer ${authEsc}'`;
  const rR = await fetch(env.SSHMCP_RELAY_URL, {
    method: "POST",
    headers: { "X-SSH-Secret": env.SSHMCP_SECRET, "Content-Type": "application/json" },
    body: JSON.stringify({ cmd: "bash", args: ["-c", resultsCmd] })
  });
  if (!rR.ok) return { ok: false, status: rR.status, body: "results gateway error", sid };
  const rJ = await rR.json().catch(() => null);
  if (!rJ || rJ.exitCode !== 0) return { ok: false, status: 0, body: `results failed: ${JSON.stringify(rJ)}`, sid };
  try {
    const data = JSON.parse(rJ.stdout || "{}");
    return { ok: true, status: 200, sid, results: data.results || [], result_count: (data.results || []).length, messages: data.messages || [] };
  } catch {
    return { ok: false, status: 0, body: `parse failed: ${(rJ.stdout||"").slice(0,200)}`, sid };
  }
}
...
var index_default = {
  async fetch(req, env, ctx) {
    ...
    if (url.pathname === "/splunk-saved-search" && req.method === "POST") {
      const body = await req.json().catch(() => null);
      if (!body || !body.name) return json({ error: "name field required" }, 400);
      const result = await splunkSavedSearchViaRelay(env, body.name, body.app || "search", body.owner || "nobody", body.earliest, body.latest);
      if (!result.ok) {
        return json({ error: "saved_search_failed", status: result.status, body: result.body, sid: result.sid }, 502);
      }
      return json({ ok: true, sid: result.sid, result_count: result.result_count, results: result.results, messages: result.messages });
    }
